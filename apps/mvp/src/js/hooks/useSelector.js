 ...
 */
export const useSelector = (selector, { querySelectorAll = false, tagSelection = false } = {}) => {
  const $ = !querySelectorAll ? document.querySelector.bind(document) : document.querySelectorAll.bind(document);
  ...
  if (!isClass && !isID && tagSelection === false) {
    ...
    console.log(`[useSelector:WARNING] -> the selector ${selector} doesn't exist`);
