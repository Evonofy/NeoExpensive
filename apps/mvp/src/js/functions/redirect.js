export const redirect = (path, { returnPathOnly = false } = {}) => {
  ...
  const pathPrefix = isGithubPages ? 'neo-expensive/packages/web' : 'packages/web';

  const host = isGithubPages ? 'https://esquemaflorescer.github.io' : window.location.origin;
