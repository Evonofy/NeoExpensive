...
(async () => {
  ...
  let sortedItems = items.sort((a, b) => (a.rating.average > b.rating.average ? -1 : 1));
  select.onchange = (event) => {
    const option = event.target.value;

    switch (option) {
      case 'leastpriced':
        sortedItems = items.sort((a, b) => (a.price.cash > b.price.cash ? 1 : -1));
        break;
      case 'mostpriced':
        sortedItems = items.sort((a, b) => (a.price.cash > b.price.cash ? -1 : 1));
        break;
      case 'bestreviewed':
        sortedItems = items.sort((a, b) => (a.rating.average > b.rating.average ? -1 : 1));
        break;
      default:
        sortedItems = items.sort((a, b) => (a.rating.average > b.rating.average ? -1 : 1));
        break;
